...
func (m *Model) startEdit(e *Entry) {
	...
	m.editFields[2].Placeholder = e.Container
...
func (m *Model) updateEdit(msg tea.KeyPressMsg) []tea.Cmd {
	...
	case "enter":
		...
		// TODO: Reimplement overrides for YAML config
		// key := tunnelKey(e)
...
// applyOverrides is a no-op - overrides are now applied during discovery.
// The YAML config system in config.go's DatabaseOverride struct handles
// per-host database overrides directly in discovery.go.
func (m *Model) applyOverrides() {
	// Override logic moved to discovery phase for YAML config.
}
...
func (m Model) calcColumns() colWidths {
	...
	cw := colWidths{
		env: 3, image: 5, host: 4, container: 6, auto: 4,
	...
	// 2-char indent + 1 space between each of 10 columns (added image) + 1 space before status.
	overhead := 2 + 10
...
func (m Model) renderEditForm() string {
	...
	var b strings.Builder
	b.WriteString("  " + headerAccent.Render(fmt.Sprintf("Configure %s/%s", e.Host, e.Container)) + "\n\n")
